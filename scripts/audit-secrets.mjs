...
const EXTRA_FILES = ["quartz.config.yaml", "SETUP.md", "MIGRATION_NOTES.md", "package.json"]
...
for (const f of findings) {
  console.log(`[${f.severity}] ${f.file}:${f.line} ${f.id} sample=${f.sample}`)
}
if (high.length > 0) {
  console.error(`audit-secrets: ${high.length} high-severity finding(s).`)
  process.exit(1)
}
console.log(`audit-secrets: ${findings.length} medium finding(s); review recommended.`)
process.exit(0)
